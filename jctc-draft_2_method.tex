...
generate quantum circuit for a wide range of configurations according to the
...
In the derivations of this paper, we use atomic units (a.u.), where the
...
\nameref{subsec:circuit-size-evaluation-results}.
...
\nameref{subsec:simulator-model}), significant simplification of the quantum circuit
...
a reversed approach: starting with the severely restricted number of qubits
...
simulations using these potentials are discussed in Section
\nameref{subsubsec:local-average-potential-results}.

\subsubsection{Method Using the Soft-core Potential}
...
discussed in Section \nameref{subsubsec:local-average-potential-results}, comparing
...
introduced in Section \nameref{sec:coulomb-potential-handling}:
...
\begin{equation}
\max_{j}\abs{\Hep(\deltar\chij)\frac{\deltat}{\hbar}} = \frac{2\deltat}{\deltar\hbar}<\pi
\end{equation}
