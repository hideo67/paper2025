...
$j$ with discretized coordinate, $\chij$, as:
\begin{equation}
\ketPsi = \sum_{j}\Psi_j\ket{j}.
\end{equation}
On the other hand, the state vector expanded with the wavenumber basis
is expressed with the wave function, $\tilde{\Psi}_{\ell}$, in k-space
such that
...
wavenumber basis, while the potential energy term is calculated in the
position basis. Using atomic units and placing the nucleus at the
...
this phase change is determined by the energy eigenvalue of the solution,
the rate is suitable as a criterion for evaluating simulation accuracy.

Using eigenvalue $E_n$ and eigenfunction $\psi_n(\bm{r})$ of the eigen value
equation,
\begin{equation}
...
(principal quantum number $n=0$) is given by a delta function, while the states
for $n>0$ can be categorized into even- and odd-parity solution. The
...
\begin{figure}
    ...
    \begin{flushleft}\small 
        ...
        $\PsiTwoD_{0,0}$ with principal quantum number $n=0$ and azimuthal
        quantum number $m=0$ (Eq. \eqref{eq:psi2D00-wave-function}). (b) Real
        part of the wave function in k-space for the same state. (c) Real part
        of the wave function in real-space for the excited state
...
as a means to drastically reduce the number of ancilla qubits, which significantly
works in small-scale models.
...
models where the total number of qubits is strictly limited, it becomes an
...
coordinates of the nucleus and the electron coincide.
...
azimuthal quantum number is $m \neq 0$ have zero amplitude at the origin; thus,
the potential value at the pole is allowed to be any finite value. However, the
$m=0$ solutions of the 2D model ($\Psi_{0,0}^{\mathrm{2D}}$ and
...
targeting the latter states.  We apply the same parameters also to the former
states for simplicity of the software.
...
because, in the 1D case, the power of the distance in the volume element is
zeroth order, which fails to cancel out the divergence of the Coulomb potential.
However, in the 1D model, since the wavefunction has zero amplitude at the
origin, assigning any arbitrary finite potential value at the singularity does
not affect the calculated energy expectation value. Therefore, for the 1D model,
we adopt a value that leads to the simplest circuit implementation.
...
hydrogen atom \cite{Eberly1991.PhysRevA.44.5997}. In this context, however, the
removal of singularity is merely a byproduct; in a 3D model where no energy
...
the literature for determining $\Delta$ rationally in this scenario.
Therefore, we propose setting $\Delta = \deltar/4$ for the 2D modelso that the
...
That is, the 2D soft-core potential used in our calculations is defined as follows:
...
tradeoff relationships that arise among each parameter under the premise that
...
center of that grid cell. As a result, the center of the simulated area is
offset by half the grid size.
...
in an increase in the truncation error $\errcutoff$. Therefore, it is necessary
to find the optimal $\deltar$ where these two errors balance out and their sum
is minimized.
...
Transform \cite{FEIT1982412}. Regarding one step of time evolution via the
Suzuki-Trotter decomposition as a sampling process, the amount of phase change
by the Hamiltonian must not exceed the phase change that corresponds to the
Nyquist frequency; that is, the maximum phase change per step must be less than
a half-period ($\pi$). This requires the following inequalities for the
potential and kinetic energy terms, respectively:
...
selected so that the Trotter error falls within an acceptable limit within that
