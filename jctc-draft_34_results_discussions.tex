...
number of gates) according to the dimensionality.
...
state preparation circuit, $\PsiSD$ and a time evolution circuit, and it is
evident that the total number of qubits is dominated by the time evolution
circuit. Furthermore, the time evolution circuit is composed of a potential
energy term circuit, $\Thetaep$, an inverse QFT circuit, a kinetic energy term
circuit, $\Thetaek$, and a QFT circuit. Figure
...
operational limits are a maximum of 27 qubits and roughly $10^7$ gates without
considering parallelization. Under these constraints, circuits were generated
using both arithmetic gate and UCRZ gate implementations, and the scaling of the
circuit size (number of qubits and number of gates) against dimensionality and
coordinate resolution was evaluated.
...
$\times$ coordinate bits). Consequently, the UCRZ circuit has fewer gates for 1D
models and 2D models with 6 or fewer coordinate bits, but for 3D models, the
arithmetic circuit requires fewer gates than the UCRZ circuit across all bit
counts.
...
\begin{figure}[ht]
    ...
    increases exponentially against input bits, making the arithmetic circuit
...
the target model and the available number of qubits and gates.
...
applying the Coulomb potential singularity treatment formulated in the
\ref{subsec:coulomb-potential-handling} section and setting the optimal simulation
...
\begin{figure}[ht]
    ...
    \begin{flushleft}\small
        ...
        \cite{kassal2008.pnas.0808245105}. The constant offset from our results
        is due to the fact that the estimation formula does not include
...
the UCRZ gate is dominant. Therefore, direct phase manipulation using UCRZ gates
is advantageous for small-scale, low-dimensional systems, while arithmetic
operations will be superior in terms of depth when performing large-scale,
high-resolution simulations in the future.
...
this category. Ultimately, the problem lies in the number of T gates these can
be approximated and decomposed into. In particular, the Rz rotation in the UCRZ
gate significantly fluctuates in T-gate consumption depending on the required
...
overhead of error correction, will be a future challenge.
...
Fig.\ref{fig:dq-tradeoff}(a)--(c) shows the evaluation results of the relationship
...
where their sum in Fig.\ref{fig:dq-tradeoff}(c) takes the minimum value. At this
time, the dimension of the simulation region is $L=14$. (In
Fig.\ref{fig:dq-tradeoff}(a), the real part of the wave function amplitude and
the boundary positions of $\pm L/2$ are indicated by vertical lines.)

On the other hand, in the case of $\nb=8$ where the number of qubits is
increased, Fig.\ref{fig:dq-tradeoff}(c) shows that the range of $\deltar$ where
the sum of errors is minimized (almost zero) is wide and flat, from $0.07 \sim
0.14$. This means that under conditions with ample computational resources such
as qubit count, the selection range for the optimal parameter broadens. Note
that outside this range, the sharp increase in error at $\deltar < 0.07$ is due
to the manifestation of the cutoff error caused by shrinking the spatial size
$L$, while the increase in error at $\deltar > 0.14$ is due to an increase in
spatial discretization error caused by coarse spatial resolution.
...
\begin{figure}[ht]
    ...
    \begin{flushleft}\small 
        ...
        show the real part values of the wave function at the intercept $y=0$
        and the computational domain boundaries ($\pm L/2$) corresponding to the
...
$2\pi/E_1^{\mathrm{2D}} \approx 28.27$, the evaluation results at $t=30$, which
corresponds to about one period, are shown in the supplementary material
...
A notable finding is the varying effect of the Trotter order depending on the
quantum numbers of the target states. For the states $\PsiTwoD_{0,0}$ and
$\PsiTwoD_{1,0}$ with the azimuthal quantum number $m=0$ (Figs.
\ref{fig:trotter-error-t1-part1}(a)--(d)), no clear difference in accuracy was
observed between different Trotter orders $\gamma$. For the 3D hydrogen atom
...
$\errdisc$ caused by the spatial resolution becomes more dominant than the temporal
error (Trotter error).
...
this is evident, when comparing Figures for three states $\PsiTwoD_{0,0}$,
$\PsiTwoD_{1,0}$, and $\PsiTwoD_{1,1}$. $\PsiTwoD_{0,0}$ with $\nb=6$ and
$\nb=9$ is shown in Figures \ref{fig:trotter-error-t1-part1}(a) ($\nb=6$) and
(b) ($\nb=9$), $\PsiTwoD_{1,0}$ with $\nb=6$ and $\nb=9$ is shown in Figures
\ref{fig:trotter-error-t1-part1}(c) ($\nb=6$) and (d) ($\nb=9$), and
$\PsiTwoD_{1,1}$ with $\nb=6$ and $\nb=9$ is shown in Figures
\ref{fig:trotter-error-t1-part2}(a) ($\nb=6$) and (b) ($\nb=9$).

It can be confirmed that Trotter error saturates around $\nT = \nTkminTwoD$ in
all cases, and the error floor after saturation is reduced when the spatial
...
is smooth at the origin. This is consistent with the fact that the spatial
discretization error $\errdisc$ is expected to decrease exponentially with
respect to $\nb$ for smooth wave functions, while it decreases only polynomially
for wave functions with cusps.
...
number becomes the representable range of this simulation, that is, the "dynamic
range." This serves as a powerful guideline for estimating the representable
state space in advance when executing simulations with limited quantum
resources.

\subsubsection{Spatial Resolution and the Plateau of Optimal Parameters}
...
system's energy error saturated when $\deltat$ was made finer than a certain
threshold. This can be interpreted as the dominant error factor of the overall
simulation shifting from the temporal error (Trotter error or sampling error) to
the spatial error (mainly spatial discretization error) caused by the coarseness
of the spatial grid $\deltar$. That is, there is a limit to the spatial accuracy
