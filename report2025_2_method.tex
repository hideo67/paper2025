%
%
% report_2025_2_method.tex
%
%
%
\documentclass[report_2025.tex]{subfiles}
\begin{document}
\section{Method}
\label{sec:method}
...
(Section \ref{subsec:simulator-model}). Subsequently, we detail the specific
methodologies for the three main challenges addressed in this paper: (1) a
circuit configuration executable on an emulator, (2) the handling of the
singularity (pole) of the Coulomb potential, and (3) a method for determining
simulation parameters that minimize errors.

\subsection{Overview of the Simulator and Computational Model}
\label{subsec:simulator-model}
We describe the configuration of the simulator circuit to be evaluated in this
report and the hydrogen atom models targeted for simulation.
...
allow users to flexibly specify and generate quantum circuit configurations
...
In the mathematical derivations of this paper, we use atomic units (a.u.), where
the elementary charge, electron mass, Bohr radius, and Planck's constant are all
set to 1. The main variables used from this section onwards are listed in Table
\ref{tab:variables}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{cc}
        ...
        $\chij$ & Discretized coordinate of grid point $j$
    \end{tabular}
    \label{tab:variables}
\end{table}

\subsubsection{Wave Function Storage Method}
...
$\Psi_j=\Psi(\chij\deltar)$ at grid point $j$ with discretized coordinate
$\chij$ as:
\begin{equation}
...
this phase change is also exactly obtained as the energy eigenvalue of the
solution, it is highly suitable as a criterion for evaluating simulation
accuracy.
...
For the 1D model, we use the analytical solution by Loudon
\cite{Loudon1959.10.1119/1.1934950}. According to Loudon, the solution for the
ground state (principal quantum number $n=0$) is a delta function. For solutions
with $n>0$, both even and odd function solutions are presented. In this paper,
we focus on the odd function solution, which is easier to handle with the
Fourier transform. The odd function solution for quantum number
$n\in\{1,2,\ldots\}$ is denoted by $\psi_n^{\mathrm{1D}}$.
...
\begin{figure}[ht]
    ...
    \begin{flushleft}\small
        Values of $\PsiOneD_1$ (Eq. \eqref{eq:psi1D1-wave-function}) at 6-bit resolution. (A) shows the wave function amplitude in real-space. (B) shows the wave function amplitude in k-space. (C) is a logarithmic scale representation of the amplitude in k-space.
    \end{flushleft}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Initial wave functions $\PsiTwoD_{0,0}$ and $\PsiTwoD_{1,0}$}
    \label{fig:initial-wavefunction-2d-00_10}
    \begin{flushleft}\small 
        (a) Ground state $\PsiTwoD_{0,0}$ with principal quantum number $n=0$ and azimuthal quantum number $m=0$ (Eq. \eqref{eq:psi2D00-wave-function}). (b) Excited state $\PsiTwoD_{1,0}$ with $n=1, m=0$ (Eq. \eqref{eq:psi2D10-wave-function}). In both, the upper row shows the real part of the wave function in real-space, and the lower row shows the real part in k-space. Both (a) and (b) have non-zero wave function values at the origin.
    \end{flushleft}
\end{figure}
...
\begin{figure}[ht]
    ...
    \begin{flushleft}\small
        Excited state $\PsiTwoD_{1,1}$ with $n=1, m=1$ (Eq. \eqref{eq:psi2D11-wave-function}). The upper row shows the real part of the wave function in real-space, and the lower row shows the real part in k-space. The wave function value at the origin is zero.
    \end{flushleft}
\end{figure}
...
multiplications on the state vector. There are several methods for representing
...
\ref{tab:emulator-circuit-configurations} in Section
\ref{subsec:circuit-size-evaluation-results}.

\subsubsection{Computing Environment}
The computer hardware and software used for the simulation calculations are as
follows:
\begin{itemize}
...
\subsection{Simulator Circuit Configuration}
...
(restricting it to 1D and 2D and fixing the nucleus, as described in Section
...
starts with the size and acceptable error of the target molecular model to
estimate the required number of qubits and gates. In contrast, this study takes
a practical approach: starting with the severely restricted number of qubits
currently available (or emulatable) and exploring/determining a circuit
configuration that can operate under that constraint. Therefore, we actively
adopt methods that can minimize the constant overhead (especially the number of
ancilla qubits) to the utmost limit for small-scale models, even if they are
unsuitable for large-scale models from an asymptotic scaling perspective.
...
particles, it is exponentially more efficient than classical computers. However,
while asymptotically efficient, the constant coefficients for the number of
ancilla qubits and gates associated with arithmetic operations are by no means
small.
...
\subsubsection{Potential Calculation Circuit Using Arithmetic Gates}
...
\begin{figure}[ht]
    ...
    \begin{flushleft}\small
        $n_1, n_2, n_3$ are registers corresponding to the discretized coordinates of electrons 1, 2, and 3, respectively, each consisting of $\nb$ bits. The slash on the qubit line indicates a register consisting of multiple qubits rather than a single qubit. $tmp$ is an ancilla qubit register. The Coulomb potential calculation circuit $\Thetaep$ must be applied to each of the three electron pairs (1,2), (2,3), and (1,3), but a single ancilla qubit register can be reused if prepared once. A qubit line passing straight through a $\Thetaep$ circuit box indicates that the qubit is not manipulated during that circuit operation.
    \end{flushleft}
\end{figure}
...
\subsubsection{Potential Calculation Circuit Using UCRZ Gates}
...
A UCRZ circuit, a variation of the UCR circuit, outputs a phase rotation angle
around the Z-axis for a target qubit depending on the state of an input
register. Because the potential term of the time evolution operator is defined
exactly as a phase rotation for each state, the operation of the UCRZ circuit
can be directly utilized as the Coulomb potential application process. This
makes it possible to pre-compute the inter-particle distance and its inverse
square root and embed them into the rotation angle parameters of a single UCRZ
circuit, thereby reducing the large number of ancilla qubits---essential in
arithmetic circuits---down to just one target qubit.

However, since the number of gates in a UCRZ circuit increases exponentially
with the number of bits in the control register, its application to large-scale
registers is impractical. Still, in verifying small-scale models where the total
number of qubits is strictly limited, it serves as an extremely effective option
capable of securing spatial resolution (qubit count) at the expense of increased
gate depth. By adopting this UCRZ circuit, we achieved a complete quantum
circuit simulation of a 2D model on an emulator in this study.
...
\begin{figure}[ht]
    ...
    \label{fig:ucrz-n3}
    \begin{flushleft}\small
    For a 3-bit input register $b_2b_1b_0$, there are 8 possible states, and 8 Rz gates act on the target qubit $t$ to provide the corresponding phase rotation for each state. $\mathrm{R}_k$ denotes $\mathrm{Rz}(\theta_k)$. CX (C-NOT) gates placed before and after each Rz gate invert the sign of the Rz gate's rotation depending on the bit string of the input register.
\end{flushleft}
\end{figure}
...
of the UCRZ circuit. Based on the potential energy $\Hp(\bm{\chi}_b)$ at the
discretized coordinate $\bm{\chi}_b$ corresponding to state $\ket{b}$, we
...
By constructing a UCRZ circuit using the $\theta_k$ obtained through this
inverse transformation, the potential energy term $\Thetaep$ of the time
...
the time evolution calculation computes the product of the potential $-1/r$ and
the wave function $\psi$ at each grid point, the value diverges at the point
where the coordinates of the nucleus and the electron coincide.
...
between the electron and the nucleus from strictly reaching zero by shifting
their coordinate grids by a half-grid spacing ($\deltar/2$)
...
\subsubsection{Method Using the Local Average Potential}

The first method, the local average potential $\Hla$, uses the original Coulomb
potential at coordinates other than the singularity and replaces it with a
finite value based on the effective radius $\reff$ only at the singularity
($r=0$).
...
The appropriate value of the effective radius $\reff$ in this approximation can
...
spatial resolution ($\deltar \ll 1$). The detailed derivation process is shown
...
The evaluation results of energy values in time evolution simulations using this
potential are discussed in Section
\ref{subsubsec:local-average-potential-results}.

\subsubsection{Approximation Formula Using Soft-core Potential}

The second method, the soft-core potential $\Hsc$, is a function that adds a
minute offset constant $\Delta$ to the norm calculation for determining
distance, smoothly avoiding divergence throughout space, and is defined as
follows \cite{Kivlichan_2017}:
...
This formula has been widely analyzed for the purpose of preventing the
divergence of the Coulomb term in Hamiltonian simulations
\cite{Childs2022quantumsimulationof} and for modeling physical systems where the
direction of electron motion is constrained \cite{Hall2009.PhysRevA.80.032507,
Fernandez_1991, LiChen2021.doi:10.1021/acs.jpca.1c00698}.

There are mainly two contexts for determining the parameter $\Delta$. One aims
to correct energy in dimensionally reduced models. For instance, it is known
that setting $\Delta=\sqrt{2}$ in a 1D hydrogen atom model yields the same
energy eigenvalue of $-0.5\ \mathrm{a.u.}$ as the ground state of a 3D hydrogen
atom \cite{Eberly1991.PhysRevA.44.5997}. However, in this context, the
regularization of the singularity is merely a byproduct, and in a true 3D space,
$\Delta=0$, leaving the divergence problem unresolved.

The primary objective in this study is the pure regularization of the
singularity. To the best of the authors' knowledge, there is no precedent that
analytically derives a physically valid $\Delta$ aligned with this objective. In
contrast, we propose setting $\Delta = \deltar/4$ so that the value of the
soft-core potential at the singularity ($r=0$) matches the value of the local
average potential analytically derived in the previous section. That is, the
soft-core potential used for calculation is defined as follows:
...
discussed in Section \ref{subsubsec:local-average-potential-results}, comparing
...
In this study, the simulation target is a rectangular (or square) domain
applying periodic boundary conditions, with a side length of $L=2^{\nb}\deltar$
(where $\nb$ is the number of coordinate qubits for each dimension, and
$\deltar$ is the grid spacing). The center of this space is set as the origin of
the coordinate system, where the nucleus is placed.
...
\subsubsection{Conditions for Time Step Based on the Sampling Theorem}
...
introduced in Section \ref{sec:coulomb-potential-handling}:
...
Because $\abs{\HlaOneD}$ is maximized at the pole ($\chij=0$), the condition becomes:
...
\subsubsection{Conditions for Time Step Based on Suzuki-Trotter Error}
...
Here, $\SN{1}(t) = \left(e^{-i{tH}_1/N}e^{-itH_2/N}\right)^N$ is the first-order time evolution operator. The minimum value of the right side of Eq. \eqref{eq:observed-trotter-error-1st} is defined as $\BN{1}$:
\begin{equation}
...
Synthesizing the above, the absolute upper limit of the time step $\deltat$ is
...
value for $\deltat$ (smallest $N$). Therefore, adopting the minimum number of
divisions within the range that contributes to the improvement of the overall
error, while assessing the balance with other error components, is the optimal
parameter determination strategy in this method.
