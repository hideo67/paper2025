...
\begin{document}
\title
{Time Evolution Simulation of 1D and 2D Hydrogen Atom Models Using Full Quantum Circuits:\\Error Analysis and Parameter Optimization}
...
emulator with fewer than 30 qubits. Specifically, we realized a simulation of a
two-dimensional single hydrogen atom model using 26 qubits (6 qubits for the
coordinate register). To the best of our knowledge, this is the first
operational report of a grid-based simulator that implements the entire process,
including the Coulomb potential, exclusively using quantum circuits.
...
potential. Furthermore, as a theoretical guideline for obtaining highly accurate
...
frequency component of the wave function. This approach provides a powerful
guideline for designing practical quantum simulations under limited resources.
\end{abstract}
...
method using $\reff=\deltar/4$ as the effective radius to determine the
