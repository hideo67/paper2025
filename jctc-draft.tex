...
constructed. (1) For small model sizes, Uniformly Controlled Rotation (UCR)
circuits were found to be a preferrable choice to calculate the Coulomb
potential compared to conventional arithmetic gates in terms of qubit count and
circuit depth. (2) As a guideline for obtaining optimal accuracy within given
...
coarseness and the domain truncation error due to a finite domain range. For the
time evolution step size, rather than relying on the formal Trotter error bound
that assumes the worst-case scenario, we observed that sampling-theorem based
bounds focusing on the maximum frequency component of the wave function are
sufficient to achieve optimal accuracy while avoiding an unnecessary increase in
