...
once FTQC is realized---mitigating the impact of noise--- and qubit count
...
not used. However, it requires more computational resources than VQE
\cite{kassal2008.pnas.0808245105}. In the grid method, the indices of equally
spaced grid points are mapped to the tensor product of computational basis
states of multi-qubit registers each corresponding to spatial coordinates. The
values of the wave function at these grid points are then stored as the
...
arithmetic operations, we utilize the Uniformly Controlled Rotation Z (UCRZ)
circuit \cite{Mottonen2004.PhysRevLett.93.130502} to reduce the required number
of qubits. The effect of UCRZ circuits is analogous to QROMs where UCRZ applies
a Z-rotation on the target qubit based on the input control qubits, while QROMs
produce bit-strings on the output register
\cite{Babbush_2018b.PhysRevX.8.041015}. For the time evolution calculation, the
standard Suzuki-Trotter decomposition is employed \cite{Zalka-rspa.1998.0162}.
Through this, we provide an example of a circuit configuration executable on an
emulator, which may serve as a foundation for basic evaluations on early-stage
FTQCs.
...
and update the phase of the wave function, that is proportional to the energy.
...
unaddressed. In this study, we propose a method to determine an effective
potential value based on the geometric average of the potential within the grid
cell containing the singularity, and we verify its validity through numerical
evaluation.
...
must also be determined. In this study, we present an integrated approach that
