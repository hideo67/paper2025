...
る。続いて、第\ref{subsec:coulomb-potential-handling-results}項にてクーロンポテ
ンシャルの特異点の扱い方に関する比較評価を行い、最後に第
\ref{subsec:parameter-selection-for-minimum-error-results}項にて誤差を最小化する
シミュレーションパラメータの決定手法に関する結果を示す。

\subsection{シミュレータ回路の構成}
\label{subsec:circuit-configuration-results}
本項では、クーロンポテンシャル計算回路を算術演算回路およびUCRZ回路で実装
...
クーロンポテンシャルの計算にUCRZ（Uniformly Controlled Rotation Z）回路を
...
数が最大27、並列性を考慮せずに数えたゲート数が $10^7$ 程度である。この制約に
対して、算術演算ゲート実装とUCRZゲート実装のそれぞれで回路を生成し、次元数や座標
分解能に対する回路規模（qubit数およびゲート数）のスケーリングを評価した。
...
\begin{figure}[ht]
    ...
    (a),(b),(c)はそれぞれ1D,2D,3Dモデルにおける所要qubit数。算術演算回路は
    1D（7bit以下）でのみ制限（赤線）に収まる一方、UCRZ回路は3D（8bit）まで制限内
    に収まる。2D/3Dでの算術演算回路のqubit数激増は、ノルム計算の複雑化に起因す
    る。
    
    (d),(e),(f)はそれぞれ1D,2D,3Dモデルにおける1000ステップの時間発展に要する2量
    子ビットゲート数。算術演算回路は次元数に対する増加が緩やかだが、UCRZ回路は入
    力ビット数に対して指数的に増加するため、3D環境下では算術演算回路の方がゲート
    数の観点から有利となる。
    \end{flushleft}
\end{figure}
...
式\eqref{eq:Halr}で定義した局所平均ポテンシャル $\Hla(r)$ において、実効距離を
...
な広がりや形状）の違いに応じて、適切な $\deltar$ の最適範囲が大きく変化すること
が確認できる。
...
$2\pi/E_1^{\mathrm{2D}} \approx 28.27$ であることを考慮した約1周期分に相当する
$t=30$ での評価結果については補助資料
\ref{smsec:parameter-selection-for-minimum-error-analysis-details}に示す。
...
\ref{fig:trotter-error-t1-part1}の(a)〜(d)）では、Trotter次数 $\gamma$ の違いに
よる明確な精度の差は見られなかった。 3次元水素原子モデルにおいてBurgarthらは、
$m=0$ の状態は原点にカスプを持つために運動エネルギーの高次モーメントが発散して高
次Trotter分解の理論的な優位性が失われることを報告している
\cite{Burgarth2024.PhysRevResearch.6.043155}。本研究の2次元モデルにおける結果は
この知見と符合する。

一方で、方位量子数 $m=1$ を持つ $\PsiTwoD_{1,1}$ の結果（図
\ref{fig:trotter-error-t1-part2}の(a)〜(b)）では、分割数 $\nT$ が小さい領域にお
いて1次（$\xiN{1}$）と2次（$\xiN{2}$）のTrotter分解の間に明確な精度差が確認でき
る。角運動量を持つ状態では原点で波動関数が滑らかとなりカスプを持たないため、高次
モーメントの発散が回避される。その結果として、高次分解本来の誤差低減効果が正常に
機能し、顕著な精度差として現れたものと解釈できる。
...
差の改善には寄与しないと言える。空間誤差との関係については、3種類の状態
（$\PsiTwoD_{0,0}$、$\PsiTwoD_{1,0}$、$\PsiTwoD_{1,1}$）をビット数を変えて比較す
ることで明確になる。 $\PsiTwoD_{0,0}$については図
\ref{fig:trotter-error-t1-part1}の(a)（$\nb=6$）と(b)（$\nb=9$）に、
$\PsiTwoD_{1,0}$については図\ref{fig:trotter-error-t1-part1}の(c)（$\nb=6$）と
(d)（$\nb=9$）に、$ \PsiTwoD_{1,1}$については図\ref{fig:trotter-error-t1-part2}
の(a)（$\nb=6$）と(b)（$\nb=9$）に、それぞれ対応するグラフを示す。

いずれのグラフにおいても、分割数 $\nT$ がサンプリング定理の下限値 $\nTkminTwoD$
を超える領域において、誤差が飽和していることが確認できる。さらに、Trotter誤差が
飽和した後の残存誤差のフロア（底）が、座標ビット数$\nb$ を$6$から$9$と3ビット増
やすことで、$1/2^3$程度に縮小していることが確認できる。
