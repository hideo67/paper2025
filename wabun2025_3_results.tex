...
よる明確な精度の差は見られなかった。Burgarthらは3次元水素原子モデルにおい
て、$m=0$ の状態は原点にカスプを持つため運動エネルギーの高次モーメントが発散し、
高次Trotter分解の理論的な優位性が失われると報告しており
\cite{Burgarth2024.PhysRevResearch.6.043155}、本研究の2次元モデルにおける結果も
この知見と符合する。
...
ることで明確になる。
$\PsiTwoD_{0,0}$については図\ref{fig:trotter-error-t1-part1}の(a)（$\nb=6$）と
(b)（$\nb=9$）に、 $\PsiTwoD_{1,0}$については図\ref{fig:trotter-error-t1-part1}
の(c)（$\nb=12$）と(d)（$\nb=9$）に、$ \PsiTwoD_{1,1}$については図
\ref{fig:trotter-error-t1-part2}の(a)（$\nb=6$）と(b)（$\nb=9$）に、それぞれ対応
するグラフを示す。

いずれのグラフにおいても、分割数 $\nT$ がサンプリング定理の下限値 $\nTkminTwoD$
を超える領域において、誤差が飽和していることが確認できる。さらに、座標ビット数
$\nb$ を$6$から$9$と3ビット増やすことで、Trotter誤差が飽和した後の残存誤差のフロ
ア（底）が$1/2^3$程度まで大きく縮小していることが確認できる。
