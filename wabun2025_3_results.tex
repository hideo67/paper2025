...
\begin{figure}[ht]
    ...
    \begin{flushleft}\small
    (a),(b),(c)は1D〜3Dモデルにおける所要qubit数。算術演算回路は1D（7bit以下）でのみ制
    限（黒破線）に収まる一方、UCRZ回路は3D（8bit）まで制限内に収まる。2D/3Dでの
    算術演算回路のqubit数激増は、ノルム計算の複雑化に起因する。
    
    (d),(e),(f)は1000ステップの時間発展に要する2量子ビットゲート数。算術演算回路は次元
...
\ref{fig:trotter-error-t1-part1}の(b)（$\nb=9$）を比較すると明確である。空間分
