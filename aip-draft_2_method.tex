...
\end{equation}
the initial wavefunction of the system is expressed as
\begin{equation}
...
(principal quantum number $n=0$) is given by a delta function, whereas the states
for $n>0$ can be categorized into even- and odd-parity solutions. The
even-parity solutions exhibit a cusp at the origin, requiring high spatial
resolution on a finite grid. Conversely, the odd-parity solutions are smooth at
the origin, placing less stringent demands on the grid resolution. Because this
study aims to assess the foundational behavior of the simulation algorithm using
a limited number of qubits, we adopt the odd-parity solutions. The odd-parity
solution for the quantum number $n \in \{1, 2, \dots\}$ is denoted by
$\psi_n^{\mathrm{1D}}$.
...
\begin{figure}
    \centering
    \includegraphics[width=8.5cm]{paper2025_figs/aip/analytic1D/t_00.000.png}
...
For the 2D model, we use the analytical solution described by Parfitt
...
Although method (b) is straightforward, the steep gradient of the potential near
